...
function handleListAktivitaeten(PDO $db, array $config, array $input): void
{
    ...
    $schuljahrId = isset($input['schuljahr_id']) ? (int) $input['schuljahr_id'] : null;
    if (!$schuljahrId) {
...
function handleExportAktivitaeten(PDO $db, array $config, array $input): void
{
    ...
    $schuljahrId = isset($input['schuljahr_id']) ? (int) $input['schuljahr_id'] : null;
    if (!$schuljahrId) {
        $row = $db->query('SELECT id, label FROM schuljahre WHERE aktiv = 1 LIMIT 1')->fetch();
        $schuljahrId = $row ? (int) $row['id'] : null;
        $label = $row['label'] ?? 'export';
    } else {
        $row = $db->prepare('SELECT label FROM schuljahre WHERE id = :id');
...
function handleExportCsv(PDO $db, array $config, array $input): void
{
    ...
    if (!$schuljahrId) {
        $row = $db->query('SELECT id FROM schuljahre WHERE aktiv = 1 LIMIT 1')->fetch();
        $schuljahrId = $row ? (int) $row['id'] : null;
    }
    ...
    $schuljahrLabel = $db->prepare('SELECT label FROM schuljahre WHERE id = :id');
    ...
    $dateiname = 'schuljahreswechsel_' . preg_replace('/[^a-z0-9]/i', '_', $label) . '.csv';
